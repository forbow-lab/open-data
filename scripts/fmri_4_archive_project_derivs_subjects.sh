...
SCRATCH_DIR_SSD=$SCRATCH/fmri/$PROJECT

if [ ! -d "$SCRATCH_DIR_SSD/$SITE/derivatives/fmriprep" ]; then
	echo " *** ERROR: could not find derivatives for $SCRATCH_DIR_SSD/$SITE/derivatives/fmriprep/"
	Usage
fi
cd $SCRATCH_DIR_SSD/
fp_tar_dir=$SCRATCH_DIR_SSD/deriv_archives/${PROJECT}_${SITE}_derivatives_fmriprep
fs_tar_dir=$SCRATCH_DIR_SSD/deriv_archives/${PROJECT}_${SITE}_derivatives_freesurfer
...
cd $SCRATCH_DIR_SSD/$SITE/derivatives/fmriprep/
...
cd $SCRATCH_DIR_SSD/$SITE/derivatives/freesurfer/
...
## --- get slurm_logs
cd $SCRATCH_DIR_SSD/deriv_archives/
if [ ! -r "${PROJECT}_${SITE}_slurm_logs.tar.gz" ]; then
	echo " + compressing $PROJECT_DIR_TAPE/slurm_logs/${PROJECT}_${SITE} -> `pwd`/${PROJECT}_${SITE}_slurm_logs.tar.gz"
	tar -vvc --use-compress-program="pigz -p 8" -f ./${PROJECT}_${SITE}_slurm_logs.tar.gz $PROJECT_DIR_TAPE/slurm_logs/${PROJECT}_${SITE}/ >./${PROJECT}_${SITE}_slurm_logs.index
...
if [ ! -r "${PROJECT}_${SITE}_resource_monitor_files.tar.gz" ]; then
	echo " + compressing $PROJECT_DIR_TAPE/slurm_logs/${PROJECT}_${SITE} -> `pwd`/${PROJECT}_${SITE}_slurm_logs.tar.gz"
	tar -vvc --use-compress-program="pigz -p 8" -f ./${PROJECT}_${SITE}_resource_monitor_files.tar.gz $SCRATCH_DIR_SSD/${SITE}/resource_monitor_files/ >./${PROJECT}_${SITE}_resource_monitor_files.index
...
cd $SCRATCH_DIR_SSD/
mkdir -p $PROJECT_DIR_TAPE/$PROJECT/Tar_DERIVS/
rsync -auxvH --no-p --no-g ./deriv_archives/ $PROJECT_DIR_TAPE/$PROJECT/Tar_DERIVS/deriv_archives/
## nohup rsync -axuH --no-p --no-g $SCRATCH_DIR_SSD/deriv_archives/ /project/6009072/fmri/${PROJECT}/Tar_DERIVS/deriv_archives/ >>logs/nohup_rsync_deriv_archives_to_project_20210905.txt 2>&1 &
